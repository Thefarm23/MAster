...
class Isotherms(object):
    ...
    def set_inlet(self, x=x_iso, y=y_iso):
        ...
        self.x = x
        self.y = y
        self.xfit = np.linspace(min(self.x), max(self.x), 50)
    ...
    def assess_fit(self):
        y_observed = self.y
        y_langmuir = self.langmuir_curve(self.x)
        y_freundlich = self.freundlich_curve(self.x)
        y_temkin = self.temkin_curve(self.x)
        y_toth = self.toth_curve(self.x)
        y_sips = self.sips_curve(self.x)
        y_DR = self.dr_curve(self.x)
        n = len(y_observed)
